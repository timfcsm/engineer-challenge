import { Policie } from '../api/models/policies'

export const policies: Policie[] = [
  {
    "id": "fb4aead9-bda2-4d16-8733-8d95e18c136a",
    "provider": "BARMER",
    "insuranceType": "HEALTH",
    "status": "PENDING",
    "startDate": "2017-04-26T05:32:06.000Z",
    "endDate": null,
    "customer": {
      "id": "51f924e3-e79f-47f4-a057-dc119491894b",
      "firstName": "Cyrillus",
      "lastName": "Biddlecombe",
      "dateOfBirth": "1978-12-03T06:33:17.000Z"
    }
  },
  {
    "id": "2062fd43-816f-4c5c-80cf-0f8267ece0ef",
    "provider": "BARMER",
    "insuranceType": "LIABILITY",
    "status": "PENDING",
    "startDate": "2015-01-13T04:52:15.000Z",
    "endDate": null,
    "customer": {
      "id": "2fca2188-898d-4409-9e24-1bda5f565ff1",
      "firstName": "Brandy",
      "lastName": "Harbour",
      "dateOfBirth": "1985-02-28T12:51:27.000Z"
    }
  },
  {
    "id": "2b87a66f-0d28-411d-9c9f-2f0d74e9b60a",
    "provider": "AOK",
    "insuranceType": "HEALTH",
    "status": "DROPPED_OUT",
    "startDate": "2014-07-14T00:54:34.000Z",
    "endDate": null,
    "customer": {
      "id": "cc0d293a-2a44-4c43-9d04-9ae415456aba",
      "firstName": "Ailina",
      "lastName": "Harber",
      "dateOfBirth": "1993-01-20T02:51:20.000Z"
    }
  },
  {
    "id": "5021669c-071b-4c80-9581-5b57ae755093",
    "provider": "AOK",
    "insuranceType": "HEALTH",
    "status": "PENDING",
    "startDate": "2020-07-21T19:40:35.000Z",
    "endDate": null,
    "customer": {
      "id": "de11551b-b8a3-47b6-bdfc-10c8038f1c1d",
      "firstName": "Aguste",
      "lastName": "Bilsford",
      "dateOfBirth": "1997-04-24T11:26:05.000Z"
    }
  },
  {
    "id": "6e3067e1-ba34-46c2-9f09-19ca3f061bee",
    "provider": "BARMER",
    "insuranceType": "HOUSEHOLD",
    "status": "ACTIVE",
    "startDate": "2013-03-30T19:27:54.000Z",
    "endDate": null,
    "customer": {
      "id": "a0b82ecf-74be-40ea-9c3a-4b97df5bdd26",
      "firstName": "Haydon",
      "lastName": "Ballay",
      "dateOfBirth": "2006-12-04T01:13:38.000Z"
    }
  },
  {
    "id": "57ba733b-4333-4608-beeb-48895f10a68e",
    "provider": "AOK",
    "insuranceType": "HEALTH",
    "status": "CANCELLED",
    "startDate": "2021-01-12T19:24:41.000Z",
    "endDate": null,
    "customer": {
      "id": "13f48d7e-190a-4590-84cd-f0d735dad9f9",
      "firstName": "Brandyn",
      "lastName": "Argyle",
      "dateOfBirth": "2006-03-17T03:35:43.000Z"
    }
  },
  {
    "id": "0ec24d60-90f3-4d29-af9b-99c30a0e1756",
    "provider": "BARMER",
    "insuranceType": "HEALTH",
    "status": "ACTIVE",
    "startDate": "2018-05-11T11:56:51.000Z",
    "endDate": null,
    "customer": {
      "id": "8cf41037-92f9-4587-b819-b985302d787b",
      "firstName": "Tani",
      "lastName": "Erasmus",
      "dateOfBirth": "2022-03-06T08:51:11.000Z"
    }
  },
  {
    "id": "5c485e5f-feb0-4108-ae9c-2a602534f335",
    "provider": "TK",
    "insuranceType": "HOUSEHOLD",
    "status": "PENDING",
    "startDate": "2013-01-25T04:14:34.000Z",
    "endDate": null,
    "customer": {
      "id": "f5249194-6ec8-44d6-a500-ff1154f39377",
      "firstName": "Galvan",
      "lastName": "Suggey",
      "dateOfBirth": "1992-05-23T18:12:40.000Z"
    }
  },
  {
    "id": "6865b801-ea1a-42bd-85c9-5910643557d3",
    "provider": "TK",
    "insuranceType": "HEALTH",
    "status": "ACTIVE",
    "startDate": "2012-09-24T09:55:17.000Z",
    "endDate": null,
    "customer": {
      "id": "2087683a-e415-4f50-b305-5846042c0f7d",
      "firstName": "Rozelle",
      "lastName": "Nipper",
      "dateOfBirth": "2005-08-12T07:01:40.000Z"
    }
  },
  {
    "id": "6c945f4f-4527-4382-839c-ba33528fa2eb",
    "provider": "AOK",
    "insuranceType": "HEALTH",
    "status": "PENDING",
    "startDate": "2020-05-02T05:53:46.000Z",
    "endDate": null,
    "customer": {
      "id": "56cac939-6e9a-46f4-808f-bbeeb7aa4438",
      "firstName": "Flossie",
      "lastName": "Camings",
      "dateOfBirth": "2004-06-15T14:20:52.000Z"
    }
  },
  {
    "id": "63441dcf-5c64-4a1e-aeb8-daff2145f2c1",
    "provider": "BARMER",
    "insuranceType": "HOUSEHOLD",
    "status": "PENDING",
    "startDate": "2022-01-12T17:47:41.000Z",
    "endDate": null,
    "customer": {
      "id": "06c7a4cd-1c15-48f5-9a80-d230351de18d",
      "firstName": "Derril",
      "lastName": "Gildersleeve",
      "dateOfBirth": "2006-02-23T00:10:07.000Z"
    }
  },
  {
    "id": "a092ee25-c268-4dd5-b77f-1cc7a85b69f4",
    "provider": "TK",
    "insuranceType": "HEALTH",
    "status": "ACTIVE",
    "startDate": "2012-07-04T15:31:29.000Z",
    "endDate": null,
    "customer": {
      "id": "42b6e6a8-0909-4082-a0e8-93959e4b482f",
      "firstName": "Amanda",
      "lastName": "McPherson",
      "dateOfBirth": "1977-10-16T17:54:53.000Z"
    }
  },
  {
    "id": "61ad93e7-5608-4f85-bf91-8e8a2fd4ead2",
    "provider": "TK",
    "insuranceType": "HEALTH",
    "status": "PENDING",
    "startDate": "2012-08-01T03:25:59.000Z",
    "endDate": null,
    "customer": {
      "id": "16c5a0e9-314d-47db-b48b-71d8b241c2e2",
      "firstName": "Garnette",
      "lastName": "Benda",
      "dateOfBirth": "1986-12-29T08:02:10.000Z"
    }
  },
  {
    "id": "9fe3aada-b39b-4ff7-a4d0-eeddafdee598",
    "provider": "DAK",
    "insuranceType": "LIABILITY",
    "status": "DROPPED_OUT",
    "startDate": "2014-02-22T06:02:58.000Z",
    "endDate": null,
    "customer": {
      "id": "12053410-1cf0-45ed-b998-9238eac6a8d7",
      "firstName": "Yoko",
      "lastName": "Becker",
      "dateOfBirth": "2005-06-10T22:51:36.000Z"
    }
  },
] as unknown as Policie[]
